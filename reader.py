...
def monitor(packet):
	if IP in packet and ICMP in packet and packet[IP].src == str(address):
		...
		if packet[ICMP].type == 8: #echo-request
			...
			os.system("curl -i -XPOST 'http://138.197.141.160:8086/write?db=ixiaDatabase' --data-binary 'echo,device=" + str(hostname) + " uuid=\"" + str(uuid) + "\",major=" + str(major) + ",minor=" + str(minor) + " " + str(time.time()*1000000)[:-2] + "'")
			...
			os.system("curl -i -XPOST 'http://138.197.141.160:8086/write?db=ixiaDatabase' --data-binary 'timestamp,device=" + str(hostname) + " ts_ori=" + str(packet[ICMP].ts_ori) + ",ts_rx=" + str(packet[ICMP].ts_rx) + ",ts_tx=" + str(packet[ICMP].ts_tx) + " " + str(time.time()*1000000)[:-2] + "'")
