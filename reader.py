...
def monitor(packet):
	if IP in packet and ICMP in packet and packet[IP].src == str(address):
		...
		if packet[ICMP].type == 8: #echo-request
			...
			os.system("curl -i -XPOST 'http://138.197.134.87:8086/write?db=cengn_11_11_2016&precision=u' --data-binary 'echo,device=" + str(hostname) + " uuid=\"" + str(uuid) + "\",major=" + str(major) + ",minor=" + str(minor) + " " + str(time.time()*1000000)[:16] + "'")
			...
			os.system("curl -i -XPOST 'http://138.197.134.87:8086/write?db=cengn_11_11_2016&precision=u' --data-binary 'timestamp,device=" + str(hostname) + " ts_ori=" + str(packet[ICMP].ts_ori) + ",ts_rx=" + str(packet[ICMP].ts_rx) + ",ts_tx=" + str(packet[ICMP].ts_tx) + " " + str(time.time()*1000000)[:16] + "'")
